...
EventIDs extractCompatibleIDs(EventID goal, const EventIDs& ids) {
  ...
  copy_if(start, ids.end(), back_inserter(result), [&goal](EventID id){ return id.isCompatible(goal); });
...
EventTypeResult findGoalEventType(EventID goal, const EventTypes& types) {
  EventTypes result;
  auto pred = [goal](const EventType& eT){ return EventID(eT).isCompatible(goal); };
